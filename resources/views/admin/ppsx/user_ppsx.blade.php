        ...
        @foreach ($investment as $data)
            <tr>
                ...
                <td>{{$data['user']['name']}}</td>
                 ...
                 <td id="status{{$data->id}}">{!! $data->status==1?'<span class="alert text-success">Acknowledge</span>':' <span class=" ack btn btn-outline-danger text-light" data-id="'.$data->id.'">Acknowledge Pending</span>' !!}</td>
...
<script>
    $(document).ready(function() {
        $('.ack').click(function(e) {
            ...
            $.ajax({
                url: "{{ route('admin.ppsx.acknowledge') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                success: function(response) {
                    if (response.status == 'success') {
                        $('#status'+investment_id).html('<span class="alert text-success">Acknowledge</span>');
